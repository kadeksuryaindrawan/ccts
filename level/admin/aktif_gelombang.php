<?php  
	...
	if(isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])){
		...
		if(mysqli_num_rows($query_check) == 1){
			$query_delete= mysqli_query($connection, "UPDATE tb_gelombang SET status_gelombang = 'aktif' WHERE id_gelombang = $id");
			if($query_delete){
				$pesan = "Gelombang sukses diaktifkan!";
			}
			else{
				$pesan = "Gelombang gagal diaktifkan!";
			}
			echo "	<script>
		 				alert('$pesan');
