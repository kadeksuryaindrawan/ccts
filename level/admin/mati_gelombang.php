<?php  
	...
	if(isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])){
		...
		if(mysqli_num_rows($query_check) == 1){
			$query_delete= mysqli_query($connection, "UPDATE tb_gelombang SET status_gelombang = 'tidak aktif' WHERE id_gelombang = $id");
			if($query_delete){
				$pesan = "Gelombang sukses dimatikan!";
			}
			else{
				$pesan = "Gelombang gagal dimatikan!";
			}
			echo "	<script>
		 				alert('$pesan');
